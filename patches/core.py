...
class SLICES:
    ...
    def __init__(self, atom_types=None, edge_indices=None, to_jimages=None, graph_method='econnn', check_results=False, optimizer="BFGS",fmax=0.2,steps=100,relax_model="m3gnet"):
        ...
        if self.relax_model=="chgnet":
            with self.suppress_output():
                self.relaxer = StructOptimizer(optimizer_class="BFGS",use_device=device)
    ...
    def get_inner_p_target(self, bond_scaling=1.05):
        ...
        try:
            ...
            blist_original = blist
            blist_flip = np.flip(blist_original, 1)
            bond_weight = []
            blist_super = np.array(data['blist'], dtype=int)
            inner_p_target = np.zeros((len(blist_original), len(blist_original)))

            # Pre-process blist_super for faster lookups
            # Maps tuple(bond_atoms_1based) to its index in blist_super
            blist_super_map = {tuple(b): idx for idx, b in enumerate(blist_super)}

            # Bond processing (diagonal elements of inner_p_target)
            for i in range(len(blist_original)):
                bond_orig_tuple = tuple(blist_original[i])
                bond_flip_tuple = tuple(blist_flip[i])
                
                xtb_bond_idx = blist_super_map.get(bond_orig_tuple)
                if xtb_bond_idx is None:
                    xtb_bond_idx = blist_super_map.get(bond_flip_tuple)

                if xtb_bond_idx is None:
                    print(f'Warning: Cannot find bond {bond_orig_tuple} or its flip in xtb output (blist_super)! Skipping bond {i}.')
                    # Assign default values or raise an error
                    bond_weight.append(1.0) # Default weight
                    inner_p_target[i, i] = 1.0 # Default squared length, consider a more robust fallback
                    continue

                if np.isnan(data['vbond'][xtb_bond_idx][2]):
                    bond_weight.append(1)
                    # data['vbond'][xtb_bond_idx][2] = 1 # Avoid modifying data if only reading
                else:
                    bond_weight.append(abs(data['vbond'][xtb_bond_idx][2]))
                inner_p_target[i, i] = round((data['vbond'][xtb_bond_idx][3] * 0.529177 * 1.05 * bond_scaling)**2, 5)

            # Pre-process blist_original for angle lookups
            # Maps tuple(bond_atoms_1based) to (index_in_blist_original, sign_factor)
            # Sign factor is +1 if bond matches blist_original[idx], -1 if it matches blist_flip[idx]
            bond_to_idx_map = {}
            for i in range(len(blist_original)):
                bond_to_idx_map[tuple(blist_original[i])] = (i, 1)
                bond_to_idx_map[tuple(blist_flip[i])] = (i, -1)

            alist_original = np.array(data['alist'], dtype=int)
            colattice_inds = [[], []]
            colattice_weights = []

            # Angle processing (off-diagonal elements of inner_p_target)
            for i in range(len(alist_original)):
                # alist_original[i] is [center_atom, neighbor1, neighbor2]
                bond1_tuple = tuple(alist_original[i][[0, 1]]) # (center, neighbor1)
                bond2_tuple = tuple(alist_original[i][[0, 2]]) # (center, neighbor2)

                res1 = bond_to_idx_map.get(bond1_tuple)
                res2 = bond_to_idx_map.get(bond2_tuple)

                if res1 and res2:
                    index_x, sign1 = res1
                    index_y, sign2 = res2
                    
                    overall_sign = sign1 * sign2 # Determines orientation for dot product

                    diag_x = inner_p_target[index_x, index_x]
                    diag_y = inner_p_target[index_y, index_y]

                    if diag_x <= 1e-9 or diag_y <= 1e-9: # Check for very small or non-positive squared lengths
                        # print(f"Warning: Skipping angle {alist_original[i]} due to non-positive/small diagonal elements for bonds {index_x}, {index_y}.")
                        continue

                    inner_p_target[index_x, index_y] = overall_sign * math.sqrt(diag_x) * math.sqrt(diag_y) * math.cos(data['vangl'][i][0])
                    inner_p_target[index_y, index_x] = inner_p_target[index_x, index_y] # Symmetrize

                    colattice_inds[0].append(int(index_x))
                    colattice_inds[1].append(int(index_y))
                    colattice_weights.append(abs(data['vangl'][i][1])) # vangl params could be negative numbers
                else:
                    # print(f"Warning: Could not find one or both bonds for angle {alist_original[i]} in blist_original map. Skipping angle.")
                    pass
    ...
    def get_rescaled_lattice_vectors(self,inner_p_target,inner_p_std,lattice_vectors_std,arc_coord_std):
        ...
        num_edges = len(self.edge_indices) # Should be equal to len(inner_p_std_diag)

        # Use a small epsilon for checking non-zero diagonal elements
        epsilon_diag = 1e-9  # Threshold for considering a diagonal element as non-zero
        
        # Identify indices where inner_p_std_diag is sufficiently large
        nonzero_edge_indices = np.where(inner_p_std_diag > epsilon_diag)[0]

        scaleList = np.zeros(num_edges)

        if len(nonzero_edge_indices) > 0:
            # Calculate scales only for valid indices
            # Ensure inner_p_target diagonal elements correspond to these indices
            scales_at_nonzero_indices = (inner_p_target[nonzero_edge_indices, nonzero_edge_indices] / inner_p_std_diag[nonzero_edge_indices])**0.5
            
            scaleList[nonzero_edge_indices] = scales_at_nonzero_indices
            scale_ave_temp = np.mean(scales_at_nonzero_indices)
            
            # Fill scales for indices where inner_p_std_diag was too small
            small_diag_indices = np.where(inner_p_std_diag <= epsilon_diag)[0]
            scaleList[small_diag_indices] = scale_ave_temp
        else:
            # All inner_p_std_diag elements are near zero or zero.
            # This indicates a potentially degenerate or problematic barycentric embedding.
            scale_ave_temp = 1.0  # Default scaling factor (e.g., no scaling)
            scaleList[:] = scale_ave_temp # Apply average/default to all

        # Update self.unstable_graph based on whether all edges had sufficiently large inner_p_std_diag
        if len(nonzero_edge_indices) < num_edges:
            self.unstable_graph = True  
        else:
            self.unstable_graph = False

        # Calculate scale_vector using NumPy for clarity and potential efficiency
        abs_arc_coord_std = np.abs(arc_coord_std)  # arc_coord_std is (num_edges, 3)
        weight_sum_np = np.sum(abs_arc_coord_std, axis=0)  # Sum over edges, result is (3,)

        scaleList_reshaped = scaleList.reshape(-1, 1)  # (num_edges, 1)
        scaled_contributions = abs_arc_coord_std * scaleList_reshaped # (num_edges, 3)
        scale_sum_np = np.sum(scaled_contributions, axis=0)  # Sum over edges, result is (3,)

        scale_vector = np.ones(3)  # Default to 1.0 for dimensions with zero weight_sum
        epsilon_div = 1e-9  # For safe division
        valid_dims = weight_sum_np > epsilon_div
        scale_vector[valid_dims] = scale_sum_np[valid_dims] / weight_sum_np[valid_dims]
    ...
    def get_coordinates(arc_coord,num_nodes,shortest_path_spanning_graph,spanning):
        ...
        Args:
            ...
            shortest_path_spanning_graph (dict): A dictionary where keys are string
                representations of 1-indexed node labels (e.g., '2', '3') and values are
                lists of 1-indexed node labels (strings) representing the shortest path
                from the source node ('1') to the key node.
            spanning (list): A list of tuples representing edges in the spanning tree.
                Each tuple is expected to be (u_str, v_str, edge_label_str), where
                u_str and v_str are 1-indexed node labels (strings), and edge_label_str
                is like 'eN' (e.g., 'e1', 'e12').

        Returns:
            np.array: Fractional coordinates of atoms (0-indexed).
        """
        coordinates = np.zeros((num_nodes, 3))  # v0 (node '1') is at origin [0,0,0]

        if num_nodes <= 1:
            # For 0 nodes, returns empty. For 1 node, returns [[0,0,0]].
            return coordinates

        # Pre-process `spanning` to create a lookup map for efficient access.
        # The map's key is a frozenset of the two 1-indexed string node labels of an edge.
        # The value is a tuple: (original_from_node_str, arc_coord_0idx).
        spanning_map = {}
        for s_edge in spanning:
            u_str, v_str, edge_label_str = s_edge
            # The edge_label_str is like 'eN'. Extract 'N', convert to int, and make it 0-indexed.
            arc_idx = int(edge_label_str[1:]) - 1
            spanning_map[frozenset({u_str, v_str})] = (u_str, arc_idx)

        # Calculate coordinates for other nodes (0-indexed 1 to num_nodes-1).
        # Node '1' (0-indexed 0) is the reference at the origin.
        for target_0idx in range(1, num_nodes):
            # Convert 0-indexed target atom to 1-indexed string label for path lookup.
            target_node_1idx_str = str(target_0idx + 1)

            # Retrieve the path (list of 1-indexed string node labels) from source '1'.
            path_1idx_str_nodes = shortest_path_spanning_graph[target_node_1idx_str]
            
            current_coord_for_target = np.zeros(3)
            
            # Iterate over the edges constituting the shortest path.
            for h in range(len(path_1idx_str_nodes) - 1):
                path_edge_u_str = path_1idx_str_nodes[h]
                path_edge_v_str = path_1idx_str_nodes[h+1]
                path_edge_nodes_fset = frozenset({path_edge_u_str, path_edge_v_str})
                
                if path_edge_nodes_fset in spanning_map:
                    spanning_original_from_node_str, arc_idx = spanning_map[path_edge_nodes_fset]
                    if path_edge_u_str == spanning_original_from_node_str:
                        current_coord_for_target += arc_coord[arc_idx]
                    else:
                        current_coord_for_target -= arc_coord[arc_idx]
                else:
                    # print(f"Warning: Edge {path_edge_nodes_fset} from shortest path for target {target_node_1idx_str} not in spanning_map.")
                    pass # Current behavior: skip if not found.
            
            coordinates[target_0idx, :] = current_coord_for_target
    ...
    def to_structures(self, bond_scaling=1.05, delta_theta=0.005, delta_x=0.45,lattice_shrink=1,lattice_expand=1.25,angle_weight=0.5,vbond_param_ave_covered=0.00,vbond_param_ave=0.01,repul=True):        
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[1] Elapsed time: {elapsed:.4f} seconds")
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[2] Elapsed time: {elapsed:.4f} seconds")
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[3] Elapsed time: {elapsed:.4f} seconds")
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[4] Elapsed time: {elapsed:.4f} seconds")
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[5] Elapsed time: {elapsed:.4f} seconds")
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[6] Elapsed time: {elapsed:.4f} seconds")
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[7] Elapsed time: {elapsed:.4f} seconds")
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[8] Elapsed time: {elapsed:.4f} seconds")
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[9] Elapsed time: {elapsed:.4f} seconds")
        ...
        if debug:
            ...
            elapsed = end_time - start_time  # Seconds as float
            start_time = end_time
            print(f"[10] Elapsed time: {elapsed:.4f} seconds")
        ...
        if net.cocycle_rep is None:
            ...
            if debug:
                ...
                elapsed = end_time - start_time  # Seconds as float
                start_time = end_time
                print(f"[11] Elapsed time: {elapsed:.4f} seconds")
            ...
            if debug:
                ...
                elapsed = end_time - start_time  # Seconds as float
                start_time = end_time
                print(f"[13] Elapsed time: {elapsed:.4f} seconds")
            ...
            if debug:
                ...
                elapsed = end_time - start_time  # Seconds as float
                start_time = end_time
                print(f"[14] Elapsed time: {elapsed:.4f} seconds")
